...
class pflile:
    ...
    def encrypt(self):
        ...
        #k = triple_des(self.key, CBC, "\0\0\0\0\0\0\0\0", pad=None, padmode=PAD_PKCS5)
        self.translated = k.encrypt(self.origtext)
    ...
    def decrypt(self):
        #k = triple_des(self.key, CBC, "\0\0\0\0\0\0\0\0", pad=None, padmode=PAD_PKCS5)
        k = des(self.key, CBC, "\0\0\0\0\0\0\0\0", pad=None, padmode=PAD_PKCS5)
        self.translated = k.decrypt(self.origtext)
        print(self.translated)
        self.text.destroy()
        self.reinit()
        self.text.insert(END,self.translated)
...
def EditEN():
    ...
    editmenu.add_command(label="Input key(file)", command=obj.key_input)
